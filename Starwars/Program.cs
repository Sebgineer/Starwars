...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<Planet> result;
            result = planets;
            Console.Write("Skriv opgave nummer: ");

            switch (Console.ReadLine())
            {
                case "1":
                    result = planets.Where(p => p.Name.StartsWith("M"));
                    break;
                case "2":
                    result = planets.Where(p => p.Name.ToLower().Contains('y'));
                    break;
                case "3":
                    result = planets.Where(p => p.Name.Length > 9 && p.Name.Length < 15);
                    break;
                case "4":
                    result = planets.Where(p => p.Name[1] == 'a' && p.Name.EndsWith("e"));
                    break;
                case "5":
                    result = planets.OrderBy(p => p.RotationPeriod).Where(p => p.RotationPeriod > 40);
                    break;
                case "6":
                    result = planets.OrderBy(p => p.Name).Where(p => p.RotationPeriod > 10 && p.RotationPeriod < 20);
                    break;
                case "7":
                    result = planets.OrderBy(p => p.Name).ThenBy(p => p.RotationPeriod).Where(p => p.RotationPeriod > 30);
                    break;
                case "8":
                    result = planets.OrderBy(p => p.Name).ThenBy(p => p.SurfaceWater).ThenBy(p => p.RotationPeriod).Where(p => (p.RotationPeriod < 30 || p.SurfaceWater > 50) && p.Name.Contains("ba"));
                    break;
                case "9":
                    result = planets.OrderByDescending(p => p.SurfaceWater).Where(p => p.SurfaceWater > 0);
                    break;
                case "10":
                    result = planets.OrderBy(p => 4 * (Math.PI) * Math.Pow(p.Diameter, 2) / p.Population).Where(p => p.Diameter > 0 && p.Population > 0);
                    break;
                case "11":
                    result = planets.Except(new List<Planet>(planets.Where(p => p.RotationPeriod > 0)));
                    break;
                case "12":
                    result = planets.Except(new List<Planet>(planets.Where(p => p.RotationPeriod > 0)));
                    break;
